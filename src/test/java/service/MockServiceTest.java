...
public class MockServiceTest {
    ...
    @Test
    public void shouldOneTimeRunForChampionSetName() {
        Champion champion = mock(Champion.class);
        champion.setName("피즈");
...
//        verify(champion,timeout(200).times(1)).setName(anyString());
    ...
    public void 리산드라검색시포지션이탑이맞는지확인() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("리산드라","미드",4));
    ...
    public void List에2개이상챔피언을넣고size호출시갯수가맞는지확인(){
        ...
        champions.add(new Champion("카서스","정글",4));
    ...
    public void 가짜챔피언객체리턴() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("소라카","서포터",3));
        assertThat(mockService.findByName("애쉬").getHasSkinCount(),is(3)   );
        verify(mockRepository,times(1)).findByName("애쉬");
    }
    ...
    public void 여러개이상호출시검증(){
        ...
        when(mockService.findAllChampions()).thenReturn(champions);
        assertThat(mockService.findAllChampions().get(0).getName(),is("카서스"));
